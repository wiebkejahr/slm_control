...
####################### 1. MAKE DATASET #############################
NUM_POINTS=15000 # will do 90/10 train/validation split
...
if [ ! -f ${DATASET} ]; then
python ${OUTPUT_DIR}/create_train_data.py ${NUM_POINTS} ${TEST_NUM} ${DATASET} -r 64 --offset --mode 'sted'
else
...
NUM_EPOCHS=15
BATCH_SIZE=64
MODEL_NAME="${NAME}_eps_${NUM_EPOCHS}_lr_${LR}_bs_${BATCH_SIZE}_noise_2"
