...
def integrate_multi(rr, res):
    ...
    out_res = res #set resolution in output (output will be out_res X out_res grid)
    # NOTE: this is because the error in the code seems to be less with one odd and one even resolution
    inp_res = res #input resolution on which integration will be done (increasing this will give more accurate results
    #                                              but program will run slower)
    n = 1.518 # refractive index of media between lens and output plane
    f = 1.8 # (in mm) effective focal length of the objective
    ...
    temp_ex = aperture * sin_theta * np.sqrt(cos_theta) * ((1 + (cos_theta - 1) * cos_phi ** 2) * px + \
        ((cos_theta - 1) * cos_phi * sin_phi) * py + (-sin_theta * cos_phi) * pz )

    temp_exj = temp_ex * jacobian

    temp_ey = aperture * sin_theta * np.sqrt(cos_theta) * ((1 + (cos_theta - 1) * sin_phi ** 2 ) * py + \
        ((cos_theta - 1) * cos_phi * sin_phi) * px + (-sin_theta * sin_phi) * pz)

    temp_eyj = temp_ey * jacobian

    temp_ez = aperture * sin_theta * np.sqrt(cos_theta) * ((sin_theta * cos_phi) * px + \
        (sin_theta * sin_phi) * py + (cos_theta) * pz)

    temp_ezj = temp_ez * jacobian
    ...
    for [xxi, xi] in enumerate(xp):
        for [yyi, yi] in enumerate(yp):
            r2 = np.sqrt(xi**2 + yi**2)
            phi2 = np.arctan2(yi,xi)
            temp_exp = np.exp(1j * (phase + k*n*r2 * sin_theta * np.cos(phi-phi2) + k*n*zi*cos_theta))
            ex[xxi,yyi] = np.sum(temp_exj * temp_exp)*dx*dx
            ey[xxi,yyi] = np.sum(temp_eyj * temp_exp)*dx*dx
            ez[xxi,yyi] = np.sum(temp_ezj * temp_exp)*dx*dx

        # if (xxi%10 == 0):
        #     print (str(xxi)+"% completed.")

    e = np.abs(ex)**2 + np.abs(ey)**2 + np.abs(ez)**2
    #e = e/np.amax(e)
    e_xy = e


    ex = np.zeros([len(xp),len(zp)])*0j
    ey = np.zeros([len(xp),len(zp)])*0j
    ez = np.zeros([len(xp),len(zp)])*0j
    ...
    for [xxi, xi] in enumerate(xp):
        for [zzi, zi] in enumerate(zp):
            ...
            phi2 = np.arctan2(yi,xi)
            temp_exp = np.exp(1j*(phase + k*n*r2*sin_theta*np.cos(phi-phi2) + k*n*zi*cos_theta))
            ex[xxi,zzi] = np.sum(np.sum(temp_exj * temp_exp))*dx*dx
            ey[xxi,zzi] = np.sum(np.sum(temp_eyj * temp_exp))*dx*dx
            ez[xxi,zzi] = np.sum(np.sum(temp_ezj * temp_exp))*dx*dx
    ...
    ex = np.zeros([len(zp),len(yp)])*0j
    ey = np.zeros([len(zp),len(yp)])*0j
    ez = np.zeros([len(zp),len(yp)])*0j
    # For y-z plane
    ...
    for [zzi, zi] in enumerate(zp):
        for [yyi, yi] in enumerate(yp):
            ...
            phi2 = np.arctan2(yi,xi)
            temp_exp = np.exp(1j*(phase + k*n*r2*sin_theta*np.cos(phi-phi2) + k*n*zi*cos_theta))
            ex[zzi,yyi] = np.sum(np.sum(temp_exj * temp_exp))*dx*dx
            ey[zzi,yyi] = np.sum(np.sum(temp_eyj * temp_exp))*dx*dx
            ez[zzi,yyi] = np.sum(np.sum(temp_ezj * temp_exp))*dx*dx

        # if (zzi%10 == 0):
        #     print (str(zzi)+"% completed.")
    ...
    # print(np.stack((e_xy, e_xz, e_yz), axis=0).shape)
    # print(np.stack((e_xy, e_xz, e_yz), axis=1).shape)
    # print(np.stack((e_xy, e_xz, e_yz), axis=2).shape)
    return np.stack((e_xy, e_xz, e_yz), axis=0)



def integrate_3D(rr, res):
    ###### Set variables #########
    output_screen_size = 0.7 # (in micrometeres) set bounds of output aperture
    z_extent = 0.7 # (in micrometeres) extent in z-direction for XZ and YZ plane results
    out_res = res #set resolution in output (output will be out_res X out_res grid)
    ...
    ex = np.zeros([len(zp),len(yp),len(xp)])*0j
    ey = np.zeros([len(zp),len(yp),len(xp)])*0j
    ez = np.zeros([len(zp),len(yp),len(xp)])*0j
    ...
    for [zzi, zi] in enumerate(zp):
        for [yyi, yi] in enumerate(yp):
            for[xxi, xi] in enumerate(xp):
                ...
                phi2 = np.arctan2(yi,xi)
                temp_exp = np.exp(1j*(phase + k*n*r2*sin_theta*np.cos(phi-phi2) + k*n*zi*cos_theta))
                ex[zzi,yyi,xxi] = np.sum(np.sum(temp_exj * temp_exp))*dx*dx
                ey[zzi,yyi,xxi] = np.sum(np.sum(temp_eyj * temp_exp))*dx*dx
                ez[zzi,yyi,xxi] = np.sum(np.sum(temp_ezj * temp_exp))*dx*dx
