...
class Sub_Pattern_Zernike(Sub_Pattern):
    ...
    def compute_pattern(self, update = True):
        ...
        data_x = pcalc.create_zernike(self.size, self.order[0], self.radnorm)
        data_y = pcalc.create_zernike(self.size, self.order[1], self.radnorm)
        self.data = self.xgui.value() * data_x + self.ygui.value() * data_y    
        if update:
            self.daddy.update()
        return self.data 
...
class Aberr_Pattern(QtWidgets.QWidget):
    ...
    def create_gui(self, p, pa):
        ...
        gui.addLayout(self.astig.create_gui(pa["astig"], defset, layout = 'h'), 0,0,1,2)
        ...
        gui.addLayout(self.coma.create_gui(pa["coma"], defset, layout = 'h'), 1,0,1,2)  
        ...
        gui.addLayout(self.sphere.create_gui(pa["sphere"], defset, layout = 'h'), 2,0,1,2)
        ...
        gui.addLayout(self.trefoil.create_gui(pa["trefoil"], defset, layout = 'h'), 3,0,1,2)
